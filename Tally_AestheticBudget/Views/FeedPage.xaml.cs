...
public partial class FeedPage : ContentPage
{
    ...
    private DataTemplate BuildCardTemplate()
    {
        return new DataTemplate(() =>
        {
            ...
            // Full-bleed image — no fixed HeightRequest, natural aspect ratio
            var photo = new Image { Aspect = Aspect.AspectFill };
            ...
            var groceryBadge = new Border
            {
                StrokeThickness = 0,
                BackgroundColor = Color.FromArgb("#1Fff6b6b"),
                Padding = new Thickness(6, 2),
            };
            groceryBadge.StrokeShape = new Microsoft.Maui.Controls.Shapes.RoundRectangle
            {
                CornerRadius = new CornerRadius(980)
            };
            groceryBadge.SetBinding(Border.IsVisibleProperty, "IsGroceryGroup");
            var groceryCount = new Label { FontSize = 10 };
            groceryCount.SetBinding(Label.TextProperty, "GroceryItemCountLabel");
            groceryCount.SetDynamicResource(Label.TextColorProperty, "AccentColor");
            groceryBadge.Content = groceryCount;
            catRow.Children.Add(groceryBadge);
